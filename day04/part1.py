...
def compute(input_str: str) -> int:
    ...
    for line in content:
        ...
        if (second_elve[0] <= first_elve[0] <= second_elve[1]) and (
            second_elve[0] <= first_elve[1] <= second_elve[1]
        ):
            c += 1
            continue
        if (first_elve[0] <= second_elve[0] <= first_elve[1]) and (
            first_elve[0] <= second_elve[1] <= first_elve[1]
        ):
            c += 1
            continue
    return c
